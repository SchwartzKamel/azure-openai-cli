...
> `:aitldr`, `:aiexp`, `:aic`, `:ai`, `:aishort`, `:aiyml`) routes to
> Azure with a 2-3 s budget and no local footprint. The local-NIM path
> is a **later upgrade** for users on Blackwell-class hardware who want
> sub-second `:aifix` / `:airw`;
