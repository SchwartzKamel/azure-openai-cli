...
**Status:** Proposed — *awaiting user sign-off before FR-020 is drafted*
**Date:** 2026-04-23
**Authors (roundtable):** Kramer, Morty Seinfeld, Newman, Jerry, Costanza, Jackie Chiles, Kenny Bania
**Collator:** rt-collate (synthesis pass)
**Related:**
...
Prospective: **FR-020 — NVIDIA NIM Provider** (not yet written; this ADR gates that work)

---

## 1. TL;DR

The user asked whether az-ai should integrate `nvidia/Gemma-4-31B-IT-NVFP4`. The 7-agent
roundtable unanimously recommends **one path only — NVIDIA NIM over HTTP, layered on
FR-018's provider abstraction, no bundling of images or weights** — with the pivot that
the user's own rig (RTX PRO 3000 Blackwell, 12 GB VRAM) **cannot run the 31B model**, so
the practical reference model becomes `nvidia/Gemma-4-9B-IT-NVFP4`. What's needed from
the user: sign-off on scope, sequencing (FR-018 first, FR-020 paired with FR-019), and
the defer-vs-flagship call that Morty and Costanza disagree on.

## 2. Context

- **User ask:** "Can az-ai talk to `nvidia/Gemma-4-31B-IT-NVFP4`? And if yes, how —
  without turning az-ai into a GPU runtime?"
- **Why this is a unique integration problem:** NVFP4 is NVIDIA's 4-bit floating-point
  format designed for Blackwell FP4 tensor cores. *Neither `llama.cpp` nor `gemma.cpp`
  supports it.* FR-018 and FR-019 — the local-model paths already in flight — therefore
  do **not** transitively cover NVFP4. A separate adapter is required, or NVFP4 is
  simply unreachable from az-ai.
- **Constraint inherited from FR-018:** we have (or will soon have) a provider
  abstraction that speaks OpenAI-compatible HTTP + configurable base URL + bearer
  token. Anything we add for NVFP4 must live inside that abstraction; we will not
  introduce a second runtime concept.
- **Constraint inherited from FR-019:** Gemma is already part of our product story on
  commodity CPUs via gemma.cpp. A NIM path would extend that story to Blackwell
  silicon, giving Gemma coverage "from laptop to rack."
- **Constraint inherited from az-ai's own posture:** we are a 40 MB AOT Alpine binary.
  We do not ship GPU drivers, CUDA, Triton, or TensorRT. Anything that requires those
  must live behind a socket.

## 3. Hardware Reality Check (the memos did not have this; it reshapes everything)

The roundtable memos were written against NVIDIA's published numbers for the **31B**
NVFP4 variant on datacenter Blackwell (B200, H200). Then we actually measured the dev
rig:

| Attribute | Dev rig (WSL2) | 31B NVFP4 needs | Fits? |
|---|---|---|---|
| GPU | RTX PRO 3000 Blackwell (mobile/workstation) | Blackwell-class | ✅ |
| VRAM | **12 GB** | ≥ 18–20 GB weights + KV cache | ❌ |
| Host RAM | 31 GB | — | ✅ |
| CPU | Intel Core Ultra 7 265H | — | ✅ |
| Driver | CUDA 12.8 | ≥ 12.4 | ✅ |
| Docker + NVIDIA Container Toolkit | **not installed** | required | ⚠ install step |

**Implication:** the 31B model does not fit. On a 12 GB Blackwell mobile card, the
models that *do* fit are:

- `nvidia/Gemma-4-9B-IT-NVFP4` — **~5 GB weights, leaves comfortable KV headroom. This
  is our reference demo model.**
- `nvidia/Gemma-4-2B-IT-NVFP4` — ~1.2 GB, a trivial smoke-test target.
- A 27B-class variant only with aggressive context reduction and a tight KV budget
  — not recommended for a demo.

**Consequence for the rest of this ADR:**

- Morty's break-even math must be re-run against the 9B's lower per-token cost and
  much lower VRAM footprint. The "$2,000 GPU" case in Morty's memo does not describe
  the user's actual hardware; the dev rig's card is a workstation SKU already
  amortized into the laptop purchase. Morty's *framework* still holds; the *numbers*
  need a 9B row added.
- Bania's published-number baseline table (B200, 31B) becomes the "aspirational
  datacenter tier." A second row for "RTX PRO 3000 Blackwell, 9B" needs manual bench
  data before any claim is published. **Every 31B number in Bania's memo is
  unreproducible on the user's rig.**
- Costanza's "sub-150ms TTFT flagship" pitch survives — arguably *improves* — because
  the 9B model is faster, and "runs on the laptop you already have" is a stronger
  story than "runs on a $40k GPU you don't."
- Kramer's NIM recipe is unchanged in shape; only the image tag and the model id
  change. `nvcr.io/nim/nvidia/gemma-4-9b-it-nvfp4:<digest>` replaces the 31B tag as
  the documented default.
- The 31B variant remains in the docs as an "if you have a B200 / dual-H200 rack,
  here's how to point az-ai at it" example. It is the *cost model exemplar*, not the
  demo target.

The "Blackwell-on-laptop developer" is now the primary ICP for FR-020. Everything
downstream follows from that.

## 4. Roundtable Summary Table

| Agent | Verdict | Key condition | Dissent / caveat |
|---|---|---|---|
| **Kramer** (eng) | ✅ Build it — NIM only | Half-day of work *after FR-018 lands*; config + docs, no new C# | Tool-calling / `--agent` mode unverified on Gemma-4 NVFP4 until smoke-tested |
| **Morty** (FinOps) | 🟡 **Defer** | Only greenlight when a user shows ≥10M tok/mo premium-tier + Blackwell silicon + NVAIE willingness | "Local is not free" — hidden NVAIE + ops + idle costs; revisit the defer call if this becomes a flagship demo regardless of volume |
| **Newman** (sec) | ✅ Conditional go | All 6 non-negotiables ship with the FR or the FR does not land | Non-loopback endpoints, NIM's default no-auth posture, and metadata-endpoint SSRF are the three loudest risks |
| **Jerry** (DevOps) | ✅ Ship, NIM-only | No bundling, compose example, pin image tag (digest ideally), mock NIM in CI | TensorRT-LLM direct is "a trap" for AOT — rejected outright |
| **Costanza** (PM) | ✅ Ship — paired with FR-019 | After FR-018, one release telling one "Gemma everywhere" story | The prestige-demo angle (NVIDIA + MSFT joint keynote slide) is the business case; mainline adoption numbers will be small and that is fine |
| **Jackie** (legal) | ✅ Ship the client, **never** the server | HTTP boundary keeps MIT clean; no bundling of NIM image or Gemma weights, ever; in-terminal notice on first use; no download helper | NVAIE is a de facto commercial dependency for prod — must be disclosed plainly |
| **Bania** (perf) | 🟡 Conditional | Real-GPU perf is **manual + nightly self-hosted**, never a merge gate. Mock-NIM bench gates *our* plumbing at 5%/10% | Every 31B number in the memo is now unreproducible on the user's 12 GB rig; all claims need a 9B re-bench before publication |

## 5. Synthesis — where consensus lives

**Unanimous (7/7):**

1. **NIM (OCI container, HTTP, OpenAI-compatible) is the only reasonable path.**
   - TensorRT-LLM direct linking is rejected (Jerry: unshippable AOT; Kramer: "own
     the zoo"; Newman: native-lib attack surface).
   - Raw `trtllm-build` + Triton + custom backend is rejected (Kramer: "not our
     job"; Jerry: three container runtimes too many).
   - `trtllm-serve` is acknowledged as a user-side fallback (same wire format) but
     is **not** a first-party az-ai integration target.

2. **This layers on FR-018's provider abstraction. No new first-class runtime.**
   - NIM is, from az-ai's perspective, "another `IChatProvider` with a different
     base URL and a bearer token." That is what FR-018 exists to make cheap.
   - Zero AOT impact (Jerry), zero new C# (Kramer), zero new JSON source-gen
     contexts (Kramer), one new provider preset file plus docs.

3. **az-ai MUST NOT bundle NIM images or Gemma weights.**
   - Jackie: bundling = "copyright infringement, breach of contract, tortious
     interference — a world of hurt."
   - Jerry: 20+ GB image into a 40 MB binary is architecturally absurd anyway.
   - Newman: supply chain stays on NVIDIA's side of the wire; we pin by digest and
     ingest the SBOM they publish.

**Disagreement (honest read):**

- **Morty says DEFER.** Hold the line until a real user shows Blackwell silicon +
  ≥10M tok/mo premium-tier traffic + NVAIE willingness. Until then, document the
  pattern in `cost-optimization.md` and move on.
- **Costanza says SHIP NOW, paired with FR-019.** The business case is prestige —
  the NVIDIA + Microsoft joint-keynote slide, the GTC demo booth, the Foundry
  catalog entry. Mainline adoption is irrelevant; ecosystem positioning is the
  prize.
- **Everyone else (Kramer, Newman, Jerry, Jackie, Bania) sits between the two** as
  "conditional go" — they will build it cleanly if asked, they will not lobby for it
  if asked to wait.

The tension is real and the user has to resolve it (see §8, question (e)). This is
not a technical disagreement; it is a product-strategy disagreement between
"demand-driven roadmap" (Morty) and "flagship-driven roadmap" (Costanza).

## 6. Recommended Direction (the decision to sign off)

**Write FR-020 now** as a fully-specified proposal targeting **NIM only**, marked as
**depends on FR-018**, and **deferred behind FR-018 landing**. Scope it to the
"Blackwell-on-laptop developer" ICP with `nvidia/Gemma-4-9B-IT-NVFP4` as the reference
demo model. Keep the 31B variant in the docs as the "if you have datacenter
Blackwell, here's the same recipe" example — it carries Morty's cost-model math but
is not the thing az-ai ships a demo against.

Sequence the release so **FR-018 lands release N; FR-020 ships paired with FR-019 in
release N+1**, under Costanza's unified "Gemma, everywhere you have silicon — from a
MacBook (gemma.cpp) to a laptop Blackwell (NIM) to a datacenter rack (NIM)" narrative.
This preserves Costanza's prestige angle without ignoring Morty's "don't over-build
for an empty audience" concern — because the 9B-on-laptop framing gives us an
audience that actually exists (us, and anyone with a recent workstation-class
Blackwell), not just keynote attendees.

**Hard guardrails in FR-020** (lifted verbatim from the roundtable — non-negotiable):

- **No bundling, ever.** (Jackie §2, Jerry §3) No NIM image, no Gemma weights, no
  TensorRT engine plans in any az-ai artifact.
- **Digest-pinned NIM image** (`nvcr.io/nim/nvidia/gemma-4-9b-it-nvfp4@sha256:…`) in
  every doc, compose file, and CI fixture. Tag-pinning is the fallback floor only
  if NVCR digest discovery is blocked. (Newman §1, Jerry §6)
- **Bearer-token auth on the NIM endpoint**, random-generated on first run, stored
  in the OS keychain, redacted in every log path. (Newman §2)
- **SSRF allow-list (not deny-list)** for the NIM provider: `127.0.0.1/32`,
  `::1/128`, resolved `localhost` only. `--allow-lan` opt-in permits RFC1918 with a
  yellow banner. Metadata endpoints (`169.254.169.254`, `metadata.google.internal`,
  `metadata.azure.com`, `100.100.100.200`, full `169.254/16`) are **always blocked,
  even under `--allow-lan`**. Post-redirect IP re-validation; DNS-rebind pinning.
  (Newman §3)
- **First-run acknowledgment file** for Gemma ToU + NVIDIA terms at
  `~/.config/az-ai/providers/nvidia.ack.json`, SHA-256 of the ToU URLs +
  timestamp. Phrased as a *notice* ("By proceeding you acknowledge…"), not a
  contract. `--yes` and `AZ_AI_ACCEPT_THIRD_PARTY_TERMS=1` for non-interactive use.
  (Jackie §3, Newman §6)
- **HTTP-only to loopback; TLS required for non-loopback.** Plain HTTP to a remote
  host refused unless `--insecure-remote` is passed, and that flag is loud.
  (Newman §4)
- **`docker-compose.yml` example** under `examples/compose/nim-gemma-9b/`
  (primary) and `examples/compose/nim-gemma-31b/` (aspirational), both with
  digest-pinned images. (Jerry §3)
- **NOTICE + trademarks updated**, NVAIE commercial dependency disclosed in
  `docs/providers/nvidia.md`, export-control boilerplate in `SECURITY.md`. No
  HuggingFace download helper — ever. (Jackie §§4–7)

**Morty's break-even section** ships *inside* FR-020 as a documented subsection so
the user can tell at a glance when local makes economic sense. The section must
include the 9B row alongside Morty's 31B numbers — the 9B changes the marginal
arithmetic materially.

**Bania's two-tier bench strategy** becomes FR-020's testing plan:

- **Tier 1 — Mock-NIM bench, CI-gated** at the usual 5% / 10% thresholds. Catches
  *our* plumbing regressions (HTTP, auth, stream parse, retry) without a GPU.
- **Tier 2 — Real-GPU bench, manual + nightly self-hosted when available**, warning
  badge in `docs/providers/nvidia.md`: *"Perf: manual / community-reported. Not
  CI-gated."* Every published number ships with sample size, variance, and
  hardware — no bare numbers, no "gold" claims without a reproducer.

## 7. Consequences

**Positive**
- az-ai becomes reachable from NVFP4 workloads without taking on any GPU-runtime
  responsibility.
- The "Gemma everywhere" narrative becomes coherent across FR-018, FR-019, FR-020.
- Zero cold-start regression; zero AOT impact; zero new top-level CLI surface.

**Negative**
- NVAIE is a commercial dependency for production users; az-ai docs must disclose
  it prominently, which complicates the "just works locally" pitch.
- Perf claims for this provider will always be community-reported rather than
  CI-gated. We must say so on the tin.
- We inherit a documentation liability around `docker run --gpus all` hardening
  even though we do not run the container ourselves.

**Rejected alternatives**
- *TensorRT-LLM native linking.* Rejected (Jerry, Kramer, Newman): AOT-unfriendly,
  CUDA P/Invoke surface, attack surface, build-farm cost. No upside over HTTP.
- *Raw Triton + custom backend.* Rejected (Kramer): "a week of YAML and protobuf
  for zero user-visible win."
- *Bundle NIM or weights in an az-ai artifact.* Rejected (Jackie, Jerry):
  catastrophic license exposure, absurd artifact size.
- *Build a HuggingFace gated-model download helper.* Rejected (Jackie §7): makes
  az-ai an intermediary in the ToU acceptance flow. Clean hands.
- *Ship FR-020 before FR-018.* Rejected (Costanza §4): "You don't pour the second
  floor before the first."

## 8. What's needed from the user to proceed

Please answer each of the following so FR-020 can be drafted with the right scope:

1. **Approve writing FR-020 along the recommended direction in §6?** `y / n`
2. **Agree that `nvidia/Gemma-4-9B-IT-NVFP4` (not 31B) is the primary demo
   target**, with 31B retained as a datacenter-tier example in docs? `y / n`
3. **Approve sequencing — FR-018 ships first (release N), FR-020 lands paired
   with FR-019 in release N+1?** `y / n`
4. **Approve NIM-only scope — reject TensorRT-LLM direct linking for v1**, accept
   `trtllm-serve` only as a user-side fallback documented but not integrated?
   `y / n`
5. **Resolve the Morty ↔ Costanza tension — pick one:**
   - (a) Sign off on Morty's defer-until-demand gate (hold FR-020 until a real user
     shows Blackwell + ≥10M tok/mo + NVAIE willingness), **or**
   - (b) Override Morty as "build this as a flagship demo regardless of volume,
     paired with FR-019 for the Gemma-everywhere release."

## 9. Open Questions (unresolved, flag before FR-020 drafting)

- **NIM license reality.** Jackie flagged that production NIM use requires an NVAIE
  subscription (~$4,500/GPU/yr list). For the user's personal dev use the free dev
  tier applies — does that carry conditions (non-commercial, eval-only, expiry)
  that change what we can demo publicly? Needs a read of the current NVAIE dev-tier
  EULA, not a 2024 summary.
- **Docker + NVIDIA Container Toolkit install cost.** The dev rig does not have
  Docker or the NVIDIA Container Toolkit installed in WSL2 yet. Who owns that
  setup in the docs — this ADR, FR-020, or a new `docs/local-model-prereqs.md`
  that FR-018 / FR-019 / FR-020 all link to? Recommendation: a shared prereqs
  page, written once, linked thrice.
- **Ack-file UX when az-ai is called non-interactively.** Espanso and AHK call az-ai
  without a TTY. A first-run y/n prompt in that path hangs. FR-018 already has to
  solve this for its own providers; FR-020 should inherit whatever pattern FR-018
  lands on (`AZ_AI_ACCEPT_THIRD_PARTY_TERMS=1`, `--yes`, and silent-deny with a
  clear stderr pointer if neither is set).
- **Is Gemma 4 actually released at time of writing?** As of 2026-04-23 the
  `nvidia/Gemma-4-*-IT-NVFP4` cards cited in the roundtable have not been verified
  live on NGC / Hugging Face from this environment. If Gemma 4 has not shipped, the
  nearest real test target today is **`nvidia/Gemma-3-9B-IT-NVFP4`** (or the
  equivalent 27B on datacenter tier), and FR-020 should be written against
  whichever model family is *actually* available on NGC when drafting begins —
  with the model id treated as a parameter, not a hard-coded assumption.

---

## Appendix A — Full roundtable memos (verbatim)

The seven memos below are embedded in full so the user can read every voice in one
place. Source files live under
`~/.copilot/session-state/<session>/files/roundtable/<agent>.md`.
...
> flows (Azure ecosystem more mature). Anyone pre-Blackwell (emulation penalty
> collapses the story).
>
> ## Bottom line
