# v2.0.0 — Social snippets

All three link the intended post-tag release URL: `https://github.com/SchwartzKamel/azure-openai-cli/releases/tag/v2.0.0`. The tag does not exist yet; Lippman creates it at ship time.
...
AOT binary: 12.91 MB (1.456× v1, inside the 1.5× gate, no waiver).
Startup p95 inside ≤25% budget. RSS ≤ v1.

New: --estimate (offline cost, CI-safe), opt-in --telemetry, --prewarm, persona routing fully wired.
...
New in this release: an offline cost estimator (`--estimate`), opt-in OpenTelemetry (`--telemetry`), connection prewarming (`--prewarm`), and persona routing wired end-to-end with persistent per-persona memory.

Release notes, migration guide, and the dual-binary upgrade story:
https://github.com/SchwartzKamel/azure-openai-cli/releases/tag/v2.0.0
```
