...
| **8** | code-review + Newman + rubber-duck (parallel audit, round 2) | Re-audited 812d2cd. Code-review came back **clean** -- no significant issues, structural invariants and test consolidation verified. Newman found **only one LOW** finding (`:aic` missing from privacy callouts despite same secret-egress class as `:aicommit`) plus 2 INFO items. **Rubber-duck killed the proposed full form-injection fix:** WSLENV+env-var pattern fully closes the bash apostrophe injection (real fix), but the PS here-string `'@` boundary breakout cannot be closed because Espanso `{{form1.X}}` substitution happens before PowerShell parses -- runtime checks fire after attack code already ran. Honest re-scope: close what's closeable, document the rest. |
| **9** | Kramer (inline) | Round-2 fix landing. **Choice fields (`:aitone`, `:aitr`, `:aireply.tone`)**: rewrote to capture form choice via `@'...'@` here-string then `switch`-map to a hardcoded safe phrase. User input is bounded by the choice list -- the bash command line never sees user-controlled text. Eliminates the entire injection surface for choice triggers (was Newman HIGH on `:aitone` apostrophe). **Free-form `:aireply.intent` (single-line)**: captured via PS here-string, sanitized (strip `"`, `\r`, `\n`), passed to bash via `$env:AZ_AI_INTENT` + `WSLENV=AZ_AI_INTENT/u`, referenced as `"$AZ_AI_INTENT"` inside the bash `--system` arg. `multiline:false` makes `'@` boundary unreachable. **Trust-model header**: added `:aic` to the privacy callouts (Newman LOW); added a `FORM-INPUT INJECTION` section honestly documenting that multi-line free-form prompts (`:ai`, `:aiweb`, `:aiimg`) have a residual PS here-string `'@` boundary surface that cannot be closed without abandoning Espanso template substitution -- threat model is essentially self-inflicted, but users should not paste prompts from untrusted sources into multi-line AI form fields. **Lint additions**: `scripts/lint-espanso-yml.sh` now also rejects `--system '...{{form1.X}}...'` patterns (the S03E01 root-cause bug class) and forbids `Invoke-Expression` composed with form values. |
