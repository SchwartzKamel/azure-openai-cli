...
You also need a working `az-ai` on `PATH` (install from a v2 release archive, `brew install schwartzkamel/tap/az-ai`, or `scoop install schwartzkamel/az-ai`) and a populated `.env` with valid Azure OpenAI credentials. Scripts 01 and 03 hit the live API; script 02 can be demoed offline by stubbing the pipe (see its inline comments).
