...
| **crush** (Charmbracelet) [^5] | Active (successor to mods) | Go | Homebrew/Go | ✅ Single binary | ✅ MCP | ⚠️ Endpoint-based | ✅ | Charm's polished TUI/TTY-first agent | Still maturing; feature parity w/ mods not 100% |
...
| **chatblade** (npiv) [^7] | 0.7.0 — **ARCHIVED** | Python | `pip`, Arch | ❌ | ❌ | ❌ | ❌ | Early "swiss army knife"; historical footprint | **Not maintained**; upstream recommends llm/fabric |
| **llama.cpp + chat clients** | Active | C++ | Source build or `brew install llama.cpp` | ✅ | ⚠️ Tool calling via model templates | N/A (local) | ⚠️ Emerging | Best local-first perf; GGUF ecosystem | Model ops complexity; not a chat UX by itself |
...
| **OpenAI Codex CLI** [^10] | Apache 2.0; included w/ ChatGPT Plus $20/mo | Go (+Rust modules) | Single binary | ✅ | ✅ Sandboxed exec (Docker/kernel) | ❌ | ✅ | Suggest/Auto-Edit/Full-Auto; strong Python/scripting | Closed model family, ChatGPT Plus tie-in for generous quota |
| **Gemini CLI** [^10] | Active (Apache 2.0) | Go (npm distribution) | `npm i -g @google/gemini-cli` | ⚠️ Node wrapper around Go core | ✅ ReAct agent + Google Search tool | ❌ | ✅ | 1 M token context, free tier 1,000 req/day | npm install surface, Google account tie-in |
| **Claude Code** [^10] | $20 Pro / $100 Max | Proprietary (Rust/TS) | Prebuilt | ✅ | ✅ Agent Teams, MCP | ❌ (Anthropic direct or Bedrock) | ✅ | SWE-bench ~81%, multi-file refactors | Proprietary, no free tier |
| **Azure AI CLI** (`az cognitiveservices`) [^11] | Built into Azure CLI | Python (azure-cli) | `az` install | ❌ | ❌ chat — **management only** | ✅ (management plane) | ❌ | First-party Microsoft tool | Provisioning tool; **not a chat client**. Gap az-ai fills. |
