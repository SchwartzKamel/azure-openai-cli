...
| costanza-2026-05-S-2 | [s03e20-the-switch.md](exec-reports/s03e20-the-switch.md) | LOW | open | Costanza + Kramer | Resolver picks an arbitrary first compat preset when several keys are set (currently `openai > groq > together > cloudflare`). Stable and documented, but a real user with all four keys exported will get a surprising default. Consider a `default_provider` knob in `preferences.json` once the FR-014 schema gains a top-level field. | 2026-05-19 |
