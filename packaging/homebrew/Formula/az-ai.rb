class AzAi < Formula
  desc "Azure OpenAI CLI — AOT agent with Microsoft Agent Framework, personas/squads, cost estimator"
  ...
  # download URLs use the clean `az-ai-<VERSION>-<RID>` pattern again.
  # Do not invent hashes at authoring time — audits will bounce anything
  # not matching the uploaded artifact.

  on_macos do
    on_arm do
      url "https://github.com/SchwartzKamel/azure-openai-cli/releases/download/v2.0.6/az-ai-2.0.6-osx-arm64.tar.gz"
    ...
    # Docker image (`ghcr.io/schwartzkamel/azure-openai-cli/az-ai`), or
    # build from source (`dotnet publish -r osx-x64`).
  end

  on_linux do
    on_intel do
      url "https://github.com/SchwartzKamel/azure-openai-cli/releases/download/v2.0.6/az-ai-2.0.6-linux-x64.tar.gz"
    ...
    # pipeline emits az-ai-<version>-linux-arm64.tar.gz. linux-musl-x64
  ...
  def install
    ...
    # `brew info az-ai` surfaces the doc dir; users can read with:
    #   cat "$(brew --prefix az-ai)/share/doc/az-ai/NOTICE"
    doc.install "LICENSE", "NOTICE", "THIRD_PARTY_NOTICES.md"
  end

  # Versioned pin support: sibling `az-ai@2.0.0.rb`, `@2.0.1.rb`,
  # `@2.0.2.rb`, `@2.0.4.rb`, `@2.0.6.rb` formulae exist in this
  # directory for users who want `brew install az-ai@2.0.6`. Each
  # tagged release gets its own versioned sibling at tag time; the
  # unversioned formula tracks the latest published release.

  test do
    assert_equal "2.0.6", shell_output("#{bin}/az-ai --version --short").strip
  end
end
